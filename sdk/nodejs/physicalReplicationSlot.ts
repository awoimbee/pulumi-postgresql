// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

/**
 * The ``postgresql.PhysicalReplicationSlot`` resource creates and manages a physical replication slot on a PostgreSQL
 * server. This is useful to setup a cross datacenter replication, with Patroni for example, or permit
 * any stand-by cluster to replicate physically data.
 *
 * ## Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as postgresql from "@awoimbee-pulumi/postgresql";
 *
 * const mySlot = new postgresql.PhysicalReplicationSlot("my_slot", {name: "my_slot"});
 * ```
 */
export class PhysicalReplicationSlot extends pulumi.CustomResource {
    /**
     * Get an existing PhysicalReplicationSlot resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: PhysicalReplicationSlotState, opts?: pulumi.CustomResourceOptions): PhysicalReplicationSlot {
        return new PhysicalReplicationSlot(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'postgresql:index/physicalReplicationSlot:PhysicalReplicationSlot';

    /**
     * Returns true if the given object is an instance of PhysicalReplicationSlot.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is PhysicalReplicationSlot {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === PhysicalReplicationSlot.__pulumiType;
    }

    /**
     * The name of the replication slot.
     */
    public readonly name!: pulumi.Output<string>;

    /**
     * Create a PhysicalReplicationSlot resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args?: PhysicalReplicationSlotArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: PhysicalReplicationSlotArgs | PhysicalReplicationSlotState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as PhysicalReplicationSlotState | undefined;
            resourceInputs["name"] = state ? state.name : undefined;
        } else {
            const args = argsOrState as PhysicalReplicationSlotArgs | undefined;
            resourceInputs["name"] = args ? args.name : undefined;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(PhysicalReplicationSlot.__pulumiType, name, resourceInputs, opts);
    }
}

/**
 * Input properties used for looking up and filtering PhysicalReplicationSlot resources.
 */
export interface PhysicalReplicationSlotState {
    /**
     * The name of the replication slot.
     */
    name?: pulumi.Input<string>;
}

/**
 * The set of arguments for constructing a PhysicalReplicationSlot resource.
 */
export interface PhysicalReplicationSlotArgs {
    /**
     * The name of the replication slot.
     */
    name?: pulumi.Input<string>;
}
